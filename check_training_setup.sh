...
DATA_PATH="${DERM7PT_DATA_PATH:-/path/to/derm7pt/release_v0}"
if [ -d "$DATA_PATH" ]; then
    IMG_COUNT=$(ls "$DATA_PATH/images/"*.jpg 2>/dev/null | wc -l)
    echo "  [OK] Dataset found: $DATA_PATH"
    echo "  Images: $IMG_COUNT files"
    if [ -f "$DATA_PATH/meta/meta.csv" ]; then
        echo "  [OK] Metadata CSV found"
    else
        echo "  [WARN] meta/meta.csv not found"
    fi
else
    echo "  [FAIL] Dataset not found at $DATA_PATH"
    echo "  Set DERM7PT_DATA_PATH to the correct location."
    exit 1
fi
echo ""
...
if [ -f "src/models/basic_cbm.py" ]; then
    echo "  [OK] Core model found"
else
    echo "  [FAIL] src/models/basic_cbm.py not found"
    exit 1
fi

if [ -f "src/data/derm7pt_adapter.py" ]; then
    echo "  [OK] Data adapter found"
else
    echo "  [FAIL] src/data/derm7pt_adapter.py not found"
    exit 1
fi
...
if [ -f "examples/train_basic_cbm.py" ]; then
    ...
    echo "  [FAIL] examples/train_basic_cbm.py not found"
    exit 1
fi
echo ""
...
if python3 -c "import sys; sys.path.insert(0, '.'); from src.models.basic_cbm import ConceptBottleneckModel" 2>/dev/null; then
    echo "  [OK] Can import ConceptBottleneckModel"
else
    echo "  [FAIL] Cannot import ConceptBottleneckModel"
...
if python3 -c "import sys; sys.path.insert(0, '.'); from src.data.derm7pt_adapter import create_derm7pt_dataloaders" 2>/dev/null; then
    echo "  [OK] Can import derm7pt_adapter"
else
    echo "  [FAIL] Cannot import derm7pt_adapter"
...
if [ -f "train_cbm.slurm" ]; then
    echo "  [OK] SLURM script found: train_cbm.slurm"
    if [ -x "train_cbm.slurm" ]; then
        echo "  [OK] Script is executable"
    else
        echo "  [NOTE] Script not executable (not required for sbatch)"
    fi
else
    echo "  [FAIL] train_cbm.slurm not found"
    exit 1
fi
echo ""
